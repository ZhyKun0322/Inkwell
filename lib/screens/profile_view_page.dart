...
class ProfileViewPage extends StatelessWidget {
  ...
  Future<void> _addFriend(BuildContext context) async {
    ...
    try {
      // Security rules only allow editing your own profile document, so we
      // only ever update our own friendIds — a one-directional "follow".
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Couldn\'t add friend: $e')));
      }
    }
  }
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
            stream: me == null ? const Stream.empty() : FirebaseFirestore.instance.collection('users').doc(me.uid).snapshots(),
            builder: (context, mySnap) {
              final myFriendIds = List<String>.from(mySnap.data?.data()?['friendIds'] ?? []);
              final alreadyFriends = myFriendIds.contains(uid);
                ...
                child: Column(
                  children: [
                    ...
                    StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                      stream: FirebaseFirestore.instance.collection('stories').where('authorId', isEqualTo: uid).snapshots(),
